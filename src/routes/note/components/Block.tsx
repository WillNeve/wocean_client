  ...
  const formBlock = (block: noteBlockType) => {
    switch (block.type) {
      case 'p':
        return (
          <div className='w-full'>
            <p className={`${empty && focused ? '' : 'hidden'} absolute pointer-events-none w-full py-1 px-1 text-gray-500 focus:bg-wave-200 outline-none`}>
              Start typing or press '/' for commands
            </p>
            <p
              className={`w-full py-1 px-1 focus:bg-wave-200 outline-none`}
      ...
      case 'h1':
          return (
              <h1
              className={`w-full py-2 px-1 focus:bg-wave-200 text-5xl outline-none`}
              contentEditable={true}
              ref={blockRef}
              onClick={() => {requestFocusShift(index)}}
              onKeyDown={handleInput}
      ...
      case 'h2':
        return (
            <h2
            className={`w-full py-1 px-1 focus:bg-wave-200 text-3xl outline-none`}
            contentEditable={true}
            ref={blockRef}
            onClick={() => {requestFocusShift(index)}}
            onKeyDown={handleInput}
      ...
      case 'em':
        return (
            <em
            className={`block w-full py-1 px-1 focus:bg2wave-400 outline-none`}
            contentEditable={true}
            ref={blockRef}
            onClick={() => {requestFocusShift(index)}}
            onKeyDown={handleInput}
        ...
        <ul>
          {suggestedCommands.length > 0 ? (
            suggestedCommands.map((command, index) => (
              <li key={index}
              className={`${index === highlightedSuggestion ? 'font-bold bg-wave-200' : ''}
                          p-1 text-sm `}>
                {command.name}
              </li>
            ))
          ) : (
            <p className={`${index === highlightedSuggestion ? 'font-bold bg-wave-200' : ''}
